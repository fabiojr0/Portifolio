...
function Home() {
    ...
    return (
      ...
      <div className='relative'>
        ...
        <img src="./waves.png" alt="waves" className="max-md:hidden h-screen object-cover absolute right-0 top 0"/>
        <div className='absolute top-0 p-4 w-screen flex justify-between items-center'>
          ...
          <span className='flex justify-between items-center md:gap-8 max-md:w-screen'>
            ...
            <a href='https://www.linkedin.com/in/fabiojr0/' target='_blank'>
              ...
              <FontAwesomeIcon icon={theme === "dark" ? faSun : faMoon} style={iconStyles} className='cursor-pointer' onClick={()=> setTheme(theme === "dark" ? "light" : "dark")}/>
          </span>
        </div>
        <div className='w-screen px-4 md:px-8 flex justify-between md:h-screen md:items-center md:pl-16'>
          ...
          <img src={theme === "dark" ? "./portifolioLogoWhite.png" : "./portifolioLogo.png"} className='h-24 w-24'/>
        </div>
      </div>
      </div>
    )
